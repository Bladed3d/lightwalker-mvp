...
// Phase 4: DnD Kit imports for unified drag and drop context
import { 
...
export default function DailyActions2Page() {
  ...
  useEffect(() => {
    ...
    // PHASE 3 FIX: Add cleanup for any stuck drag states on mount
    const cleanupDragArtifacts = () => {
      if ((window as any).currentDraggedActivity) {
        delete (window as any).currentDraggedActivity;
      }
    ...
    // Add page visibility change handler to clean up drag states
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden') {
        // Clean up any active drag operations when page becomes hidden
        cleanupDragArtifacts();
      }
    };
  ...
  // COMMENTED OUT: FORCE DEBUG during DnD testing - What is the actual value?
  ...
  // PHASE 3 FIX: DnD state management - declared early to avoid hoisting issues
  const [isDndActive, setIsDndActive] = useState(false);
  const [dragStartTime, setDragStartTime] = useState<number | null>(null);
  const [activeId, setActiveId] = useState<string | null>(null);
  
  // Phase 4: DnD Kit sensor configuration for unified drag context
  ...
  // PHASE 3 FIX: Update live time for server status - drag-safe implementation
  useEffect(() => {
    // Initial time
    setLiveTime(new Date());
    
    // Conservative timer that pauses during drag operations
    const timer = setInterval(() => {
      // Only update if not actively dragging to prevent console crashes
      if (!isDndActive) {
        setLiveTime(new Date());
      }
    }, 30000); // 30 second intervals to minimize overhead during DnD
    
    return () => clearInterval(timer);
  }, [isDndActive]); // Restart timer when drag state changes
  ...
  // Handle activity drop from grid to timeline
  const handleActivityAdd = (activity: any, timeString: string) => {
    console.log('🎯 Activity dropped on timeline:', { activity, time: timeString });
    
    // Create a new timeline activity with the dropped time
    const newTimelineActivity = {
      ...activity,
      id: `timeline-${Date.now()}-${activity.id}`,
      scheduledTime: timeString,
      startTime: timeString,
      isDroppedActivity: true
    };
    
    // Add to timeline activities
    setAllTimelineActivities(prev => [...prev, newTimelineActivity]);
    setTimelineActivities(prev => [...prev, newTimelineActivity]);
    
    console.log('✅ Activity added to timeline at:', timeString);
  };
  ...
  // PHASE 3: DnD Kit state tracking with console crash prevention (moved up for hoisting)
  
  // Phase 4: DnD Kit handlers for unified drag context
  const handleDnDKitDragStart = React.useCallback((event: DragStartEvent) => {
    console.log('🎯 Phase 4: Unified DnD Kit drag started:', event.active.id);
    setIsDndActive(true);
    setDragStartTime(Date.now());
    setActiveId(event.active.id);
    
    // Store the dragged activity data for cross-component access
    const draggedActivity = event.active.data?.current?.activity;
    if (draggedActivity) {
      (window as any).currentDraggedActivity = draggedActivity;
      console.log('📦 Stored dragged activity:', draggedActivity);
    }
  }, []);

  const handleDnDKitDragOver = React.useCallback((event: DragOverEvent) => {
    console.log('🔄 Phase 4: Unified DnD Kit drag over:', {
      activeId: event.active.id,
      overId: event.over?.id,
      overData: event.over?.data?.current
    });
    
    // Extra debug for timeline
    if (event.over?.id === 'timeline-drop-zone') {
      console.log('🎯 DETECTED: Dragging over timeline drop zone!');
    }
  }, []);

  const handleDnDKitDragEnd = React.useCallback((event: DragEndEvent) => {
    console.log('🏁 Phase 4: Unified DnD Kit drag ended:', {
      activeId: event.active.id,
      overId: event.over?.id,
    ...
    // Note: Cross-component drops are now handled natively by individual DND Kit components
    // GamelikeTimeline and TarkovInventoryGrid handle their own drop logic
    if (event.over) {
      console.log('🎯 Phase 3: DND Kit cross-component drop completed natively');
    }
  ...
  // Phase 3: Placeholder drag end handler (actual drag handling done by DND Kit)
  const handleDragEnd = (result: any) => {
    console.log('🎯 Phase 3: Legacy handleDragEnd called (DND Kit handles actual drops):', result);
    // Note: All actual drag and drop logic is now handled by DND Kit context
    // This function remains as a placeholder for components that still expect it
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Setting up DND Kit system</p>
        </div>
      </div>
    );
  }
  ...
  return (
    // Phase 4: Unified DnD Kit Context for cross-component drag and drop
    <DnDKitContext
        ...
        <div className={`${isMobileView ? 'space-y-4 mb-4' : 'flex gap-4 mb-6'}`}>
          ...
          <div className={`${isMobileView ? 'w-full' : 'flex-1'}`}>
            ...
            onActivityEdit={(activity) => {
              console.log('🎯 Phase 3: Opening edit modal for timeline activity:', activity.title || activity.name);
              
              // Open Duration Screen
              setUIState(prev => ({ ...prev, quickActionPanel: 'edit' }));
              
              // Pass the timeline activity to the DurationScreen
              window.dispatchEvent(new CustomEvent('activityDropped', {
                detail: {
                  id: `edit-${Date.now()}-${activity.id || activity.activityId}`,
                  title: activity.title || activity.name,
                  duration: activity.duration || activity.customDuration || '15 min',
                  points: activity.points || activity.customPoints || 25,
                  icon: activity.icon,
                  category: activity.category || activity.customCategory || 'custom',
                  difficulty: activity.difficulty || 2,
                  originalDuration: activity.duration || activity.customDuration || '15 min',
                  gridSize: { w: 1, h: 1 },
                  instructions: activity.instructions || activity.notes || '',
                  isRecurring: activity.isRecurring || false,
                  recurringPattern: activity.recurringPattern,
                  scheduledTime: activity.scheduledTime || activity.time
                }
              }));
            }}
            ...
            <div className="mt-4">
              <TarkovInventoryGrid 
                ...
                isMobileView={isMobileView}
                onActivityEdit={(activity) => {
                  console.log('🚨 CRITICAL DEBUG - onActivityEdit called from main page:', activity);
                  
                  // Open Duration Screen
                  setUIState(prev => ({ ...prev, quickActionPanel: 'edit' }));
                  
                  // Pass the activity to the DurationScreen
                  window.dispatchEvent(new CustomEvent('activityDropped', {
                    detail: {
