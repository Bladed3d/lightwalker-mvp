...
async function handleReseed() {
  try {
    // Force clear ALL existing templates
    await prisma.lightwalkerTemplate.deleteMany({})
    
    console.log('Cleared all existing templates, reseeding with complete set...')
    ...
    const generalCount = allTemplates.filter(t => t.category === 'general').length
    const situationalCount = allTemplates.filter(t => t.category === 'situational').length
